...
export async function GET(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${process.env.GATEWAY_URL}/api/v1/conversations?${queryString}`, {
            method: 'GET',
            headers: {
...
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${process.env.GATEWAY_URL}/api/v1/conversations`, {
            method: 'POST',
            headers: {
                Cookie: decodeURIComponent(cookieHeader),
            },
